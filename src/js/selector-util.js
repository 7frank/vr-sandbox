...
//    modelEl=jQuery("[gltf-model]")[3];
  ...
  items.forEach(el => {
    ...
    modelEl.body.addShape(shape, new CANNON.Vec3(vPosition.x, vPosition.y, vPosition.z));

    if (debug) {
      // console.log(el);
      // console.log('position', stringifyWithPrecision(vPosition), 'size', stringifyWithPrecision(size));

      console.log('position', stringifyWithPrecision(vPosition));

      var helper = addBoundingBox(el);

      modelEl.addEventListener('collide', function (e) {
