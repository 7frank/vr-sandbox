...
float h = uWidth * texture2DLod(uHeightData, st, lod).a;
  h += uWidth/16.0 * texture2DLod(uHeightData, uWidth/64.0 * st, lod).a;
  h += uWidth/256.0 * texture2DLod(uHeightData, uWidth / 4.0 * st, lod).a;

  // Square the height, leads to more rocky looking terrain
  return h * h / uWidth*2.0;
