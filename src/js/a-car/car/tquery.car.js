...
class WorldShim {
  ...
  unhook (fn) { window.clearAnimationFrame(fn); }
}

export default class Car {
  constructor (opts) {
    this.baseUrl	= 'assets/cars/obj/';

    // handle parameters
    this._opts = extend({
      type: 'veyron',
      scale: 1.5,
      world: new WorldShim(), // tQuery.world
      el: null // el.body should handle the steering if it exists ...   instance of  physical system
    }, opts);
    ...
    // determine urls
    var bodyUrl, wheelUrl;
    var baseUrl = this.baseUrl;
    if (this._opts.type === 'gallardo') {
      bodyUrl = baseUrl + 'gallardo/parts/gallardo_body_bin.js';
      wheelUrl = baseUrl + 'gallardo/parts/gallardo_wheel_bin.js';
    } else if (this._opts.type === 'veyron') {
      bodyUrl = baseUrl + 'veyron/parts/veyron_body_bin.js';
      wheelUrl = baseUrl + 'veyron/parts/veyron_wheel_bin.js';
    ...
    this._loopCb = function (delta) {
      this._car.updateCarModel(delta, this._controlsCar, this._opts.el);
    }.bind(this);
  ...
  _addFlare () {
    ...
    // FLARES
    var flareA	= THREE.ImageUtils.loadTexture('assets/cars/images/lensflare2.jpg');
    var flareB	= THREE.ImageUtils.loadTexture('assets/cars/images/lensflare0.png');
  ...
  _setNiceMaterial () {
    var textureCube	= createCubeTexture('swedishRoyalCastle');
