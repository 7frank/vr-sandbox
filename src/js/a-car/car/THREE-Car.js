...
THREE.Car = function () {
  ...
  this.updateCarModel = function (delta, controls, el) {
    ...
    if (!(controls.moveForward || controls.moveBackward)) {
      if (this.speed > 0) {
        k = exponentialEaseOut(this.speed / this.MAX_SPEED);
        ...
        k = exponentialEaseOut(this.speed / this.MAX_REVERSE_SPEED);
...
*/
    // displacement
    if (el.body) {
      el.body.position.x += Math.sin(this.carOrientation) * forwardDelta;
